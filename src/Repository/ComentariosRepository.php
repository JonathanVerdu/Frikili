...
class ComentariosRepository extends ServiceEntityRepository
{
    ...
    public function ComentariosDelUsuario($user){
        return $this->getEntityManager()
            ->createQuery('
                SELECT c.id, p.titulo, p.id as post_id
                FROM App:Comentarios c, App:Posts p
                WHERE c.user = '.$user.' AND p.id = c.posts   
            ')->getResult();
    }
    ...
    public function MostrarComentarios($id){
        return $this->getEntityManager()
            ->createQuery('
                SELECT c.id, c.comentario, p.titulo, u.nombre
                FROM App:Comentarios c, App:Posts p, App:User u
                WHERE c.user = u.id AND p.id = c.posts AND c.posts = '.$id.' 
            ')->getResult();
    }
